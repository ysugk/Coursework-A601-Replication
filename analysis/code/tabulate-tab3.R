...
gen_summary(df_group1) %>%
  as.data.frame() %>%
  stargazer(summary = FALSE, type = "text", out = "analysis/output/tabs/tab3a.tex",
...
gen_summary(df_group2) %>%
  as.data.frame() %>%
  stargazer(summary = FALSE, type = "text", out = "analysis/output/tabs/tab3b.tex",
            digits = 2, float = FALSE, rownames = FALSE)
...
gen_summary(df_group3) %>%
  as.data.frame() %>%
  stargazer(summary = FALSE, type = "text", out = "analysis/output/tabs/tab3c.tex",
            digits = 2, float = FALSE, rownames = FALSE)

# All
gen_summary(data2) %>%
  as.data.frame() %>%
  stargazer(summary = FALSE, type = "text", out = "analysis/output/tabs/tab3d.tex",
            digits = 2, float = FALSE, rownames = FALSE)
